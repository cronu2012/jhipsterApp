<template>
  <div>
    <h2 id="page-heading" data-cy="ProductHeading">
      <span v-text="t$('jhipsterApp.product.home.title')" id="product-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('jhipsterApp.product.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'ProductCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('jhipsterApp.product.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && products && products.length === 0">
      <span v-text="t$('jhipsterApp.product.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="products">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('mainTitle')">
              <span v-text="t$('jhipsterApp.product.mainTitle')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'mainTitle'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('subTitle')">
              <span v-text="t$('jhipsterApp.product.subTitle')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'subTitle'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('description')">
              <span v-text="t$('jhipsterApp.product.description')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'description'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('price')">
              <span v-text="t$('jhipsterApp.product.price')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('createBy')">
              <span v-text="t$('jhipsterApp.product.createBy')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('createdTime')">
              <span v-text="t$('jhipsterApp.product.createdTime')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'createdTime'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('updatedTime')">
              <span v-text="t$('jhipsterApp.product.updatedTime')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'updatedTime'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('status')">
              <span v-text="t$('jhipsterApp.product.status')"></span>
        ...
        <tbody>
          <tr v-for="product in products" :key="product.id" data-cy="entityTable">
            ...
            <td>{{ product.mainTitle }}</td>
            <td>{{ product.subTitle }}</td>
            <td>{{ product.description }}</td>
            <td>{{ product.price }}</td>
            <td>{{ product.createBy }}</td>
            <td>{{ formatDateShort(product.createdTime) || '' }}</td>
            <td>{{ formatDateShort(product.updatedTime) || '' }}</td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="jhipsterApp.product.delete.question" data-cy="productDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-product-heading" v-text="t$('jhipsterApp.product.delete.question', { id: removeId })"></p>
      </div>
