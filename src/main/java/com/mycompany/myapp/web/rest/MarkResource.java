...
import com.mycompany.myapp.service.MarkQueryService;
...
public class MarkResource {
    ...
    public ResponseEntity<MarkDTO> createMark(@Valid @RequestBody MarkDTO markDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/marks/" + markDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, markDTO.getId().toString()))
            .body(markDTO);
    }
    ...
    public ResponseEntity<MarkDTO> updateMark(
        ...
        markDTO = markService.update(markDTO);
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<MarkDTO> getMark(@PathVariable("id") Long id) {
